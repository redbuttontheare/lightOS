...
function wm.createTab(name, program)
    for _, tab in ipairs(tabs) do
        if tab.name == name and not tab.dead then
            return nil
        end
    end
    ...
    if isRunning and resumeTab then
        resumeTab(tab, {})
        drawTabBar()
    end
...
local function focusTab(index)
    if not tabs[index] or index == focusedIndex or tabs[index].dead then return end
...
local function handleTabBarClick(x)
    for i, tab in ipairs(tabs) do
        if not tab.dead and x >= tab.x1 and x <= tab.x2 then
            focusTab(i)
            return
        end
    end
...
function wm.run()
    if isRunning or _G.GELAXY_ACTIVE then
        printError("Gelaxy is already running!")
        return
    end
